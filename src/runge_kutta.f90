module runge_kutta_m 
    ...
    type, abstract, public :: runge_kutta_t 
        real(real_t) :: eps 
        real(real_t), dimension(:), pointer :: yscal 
    contains 
    ...
    abstract interface
        ...
        subroutine derivsx(this, x, y, dydx)
            use real_m
            ...
            real(real_t), dimension(:), intent(in) :: y
            real(real_t), dimension(:), intent(out) :: dydx
...
contains 
    ...
    subroutine rkck_sub(this, y, dydx, x, h, yout, yerr)
        use rk_parameters_m
        implicit none 

        class(runge_kutta_t) :: this
        real(real_t), intent(in) :: x, h
        real(real_t), dimension(:), intent(in) :: y, dydx
        real(real_t), dimension(:), intent(out) :: yout, yerr
        real(real_t), dimension( size(y) ) :: ak2, ak3, ak4, ak5, ak6, ytemp
        integer :: ndum
               
        ndum = size(y, dim = 1)
        ytemp = y + b21 * h * dydx
        call this%derivs(x + a2 * h, ytemp, ak2)
        ytemp = y + h * ( b31 * dydx + b32 * ak2 )

        call this%derivs(x + a3 * h, ytemp, ak3)
        ytemp = y + h * ( b41 * dydx + b42 * ak2 + b43 * ak3 )

        call this%derivs(x + a4 * h, ytemp, ak4)
        ytemp = y + h * ( b51 * dydx + b52 * ak2 + b53 * ak3 + b54 * ak4 )

        call this%derivs(x + a5 * h, ytemp, ak5)
        ytemp = y + h * ( &
            b61 * dydx + b62 * ak2 + b63 * ak3 + b64 * ak4 + b65 * ak5 &
        )

        call this%derivs(x + a6 * h, ytemp, ak6)
        yout = y + h * ( &
    ...
    subroutine rkqs_sub(this, y, dydx, x, htry, hdid, hnext)
        class(runge_kutta_t) :: this
        real(real_t), dimension(:), intent(inout) :: y
        real(real_t), dimension(:), intent(in) :: dydx
